...
50.times do
  ...
  Property.all.sample(rand(15)).each do |prop|
    if prop.quantifiable
      SpaceProperty.create(space: space, property: prop, quantity: rand(3))
    else
      SpaceProperty.create(space: space, property: prop)
    end
  end
